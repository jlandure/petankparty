...
public class MatchResource extends DefaultGroupResource {
	...
	void doPost(Form form) {
		...
		place = PetankPlaceUtil.instance.getPlaceById(idPlace).name
		}
		def type
	...
	@Override
	void doDelete(Form form) {
		def idMatch = form.getFirstValue("idMatch")
		listUsers = PetankUserUtil.instance.getUserByGroupName(groupName)
		Match match = MatchUtil.instance.getMatchById(idMatch)
		StatUtil.instance.deleteMatch(match, listUsers)
		DefaultResource.MEMCACHE.clear()
		getResponse().redirectSeeOther(new Reference(getRootUri()+"/$groupName/classement"));
	}
	...
	void acceptRepresentation(Representation entity) {
		...
		def type = form.getFirstValue("type")
		if(type == "POST") {
			doPost(form)
		}
		if(type == "DELETE") {
		...
		html.html {
			...
			body {
				...
				p { 
					...
					br()
					a(href:getRootUri()+"/${groupName}/match/add",  "Ajouter un match")
				}
						...
						if(listMatchs != null) {
							listMatchs.each{
								...
								tr {
									...
									td(class:"special") {
										a(href:(getRootUri()+"/place/"+place.name), target:"_blank", PetankPlaceUtil.instance.getPlaceName(place))
									}
									if(first == 0) {
										//on affiche que pour le dernier match la possibilité de supprimer 
										td(class:"special") {
											a(href:(getRootUri()+"/$groupName/match/"+match.id), "Delete")
										}
									first++
									}
								}
